...
public final class Drivetrain extends Subsystem {
    ...
    public Drivetrain() {
        ...
        // Greyhill Encoders
        leftGreyhill = new Encoder(RobotMap.DRIVETRAIN_LEFT_ENCODER_CHANNEL_A,
                RobotMap.DRIVETRAIN_LEFT_ENCODER_CHANNEL_B);
        rightGreyhill = new Encoder(RobotMap.DRIVETRAIN_RIGHT_ENCODER_CHANNEL_A,
    ...
    public double getLeftMotorOutput() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highLeftTopMotor.getAppliedOutput() + 
                    highLeftMiddleMotor.getAppliedOutput()) / 2;
        } else {
            return (lowLeftTopMotor.getAppliedOutput() + 
                    lowLeftMiddleMotor.getAppliedOutput() +
                    leftBottomMotor.getAppliedOutput()) / 3;
        }
    }

    public double getRightMotorOutput() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highRightTopMotor.getAppliedOutput() + 
                    highRightMiddleMotor.getAppliedOutput()) / 2;
        } else {
            return (lowRightTopMotor.getAppliedOutput() + 
                    lowRightMiddleMotor.getAppliedOutput() +
                    rightBottomMotor.getAppliedOutput()) / 3;
        }
    }

    public double getLeftMotorCurrent() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highLeftTopMotor.getOutputCurrent() + 
                    highLeftMiddleMotor.getOutputCurrent()) / 2;
        } else {
            return (lowLeftTopMotor.getOutputCurrent() + 
                    lowLeftMiddleMotor.getOutputCurrent() +
                    leftBottomMotor.getOutputCurrent()) / 3;
        }
    }

    public double getRightMotorCurrent() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highRightTopMotor.getOutputCurrent() + 
                    highRightMiddleMotor.getOutputCurrent()) / 2;
        } else {
            return (lowRightTopMotor.getOutputCurrent() + 
                    lowRightMiddleMotor.getOutputCurrent() +
                    rightBottomMotor.getOutputCurrent()) / 3;
        }
    }

    public double getLeftMotorVoltage() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highLeftTopMotor.getBusVoltage() + 
                    highLeftMiddleMotor.getBusVoltage()) / 2;
        } else {
            return (lowLeftTopMotor.getBusVoltage() + 
                    lowLeftMiddleMotor.getBusVoltage() +
                    leftBottomMotor.getBusVoltage()) / 3;
        }
    }

    public double getRightMotorVoltage() {
        if (currentDifferentialDrive == highDifferentialDrive) {
            return (highRightTopMotor.getBusVoltage() + 
                    highRightMiddleMotor.getBusVoltage()) / 2;
        } else {
            return (lowRightTopMotor.getBusVoltage() + 
                    lowRightMiddleMotor.getBusVoltage() +
                    rightBottomMotor.getBusVoltage()) / 3;
        }
    }
}
